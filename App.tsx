...
import { ImagingPipeline } from './components/ImagingPipeline';
...
import type { Credentials, PipelineDevice, PipelineStage } from './src/types';
import { AppProvider, useAppContext } from './contexts/AppContext';
// Re-export so any legacy imports from '../App' continue to resolve.
export { TARGET_BIOS_VERSION, TARGET_DCU_VERSION, TARGET_WIN_VERSION } from './src/constants';

// ---------------------------------------------------------------------------
// Helper: parse manual CSV text into PipelineDevice objects
// ---------------------------------------------------------------------------
const parsePipelineCsv = (text: string): PipelineDevice[] => {
    const lines = text.trim().split('\n').filter(Boolean);
    const result: PipelineDevice[] = [];
    for (const line of lines) {
        const parts = line.split(',').map(s => s.trim());
        const hostname = parts[0];
        const mac = (parts[1] || '').replace(/[:\-. ]/g, '').toUpperCase();
        const ip = parts[2] || '';
        if (!hostname || mac.length !== 12) continue;
        result.push({
            id: mac,
            hostname,
            macAddress: mac,
...
// Tab button
// ---------------------------------------------------------------------------
type TabName = 'monitor' | 'runner' | 'build' | 'script' | 'remote' | 'pipeline';

const AppContent: React.FC = () => {
    const { state, dispatch } = useAppContext();
    const { runner, monitor, ui, pipeline } = state;

    const isReadyToDeploy = ui.csvFile || runner.devices.length > 0;

    const TabButton: React.FC<{ tabName: TabName; label: string; icon: React.ReactNode }> = ({ tabName, label, icon }) => (
        <button
    ...
    const completedMonitorDevices = monitor.devices.filter(d => d.status === 'Completed').length;
        ...
        <div className="min-h-screen bg-black text-gray-200 font-sans p-4 sm:p-6 lg:p-8">
            ...
            <div className="mt-6">
                <div className="flex border-b border-gray-800 flex-wrap">
                    ...
                    <TabButton tabName="pipeline" label="Imaging Pipeline" icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M3 5a1 1 0 000 2h1a1 1 0 100-2H3zM7 5a1 1 0 000 2h10a1 1 0 100-2H7zM3 9a1 1 0 000 2h1a1 1 0 100-2H3zM7 9a1 1 0 000 2h10a1 1 0 100-2H7zM3 13a1 1 0 000 2h1a1 1 0 100-2H3zM7 13a1 1 0 000 2h10a1 1 0 100-2H7z" clipRule="evenodd" /></svg>} />
            ...
            <main className="mt-8">
                ...
                {ui.activeTab === 'pipeline' && (
                    <ImagingPipeline
                        pipeline={pipeline}
                        monitorDeviceCount={completedMonitorDevices}
                        onImportFromMonitor={() => dispatch({ type: 'PIPELINE_IMPORT_FROM_MONITOR' })}
                        onAddManualDevices={(csv) => {
                            const devices: PipelineDevice[] = parsePipelineCsv(csv);
                            if (devices.length > 0) dispatch({ type: 'PIPELINE_ADD_DEVICES', payload: devices });
                        }}
                        onRunStage={(stage: PipelineStage) => dispatch({ type: 'RUN_PIPELINE_STAGE', payload: stage })}
                        onSetPackage={(packageId) => dispatch({ type: 'PIPELINE_SET_PACKAGE', payload: packageId })}
                        onClear={() => dispatch({ type: 'PIPELINE_CLEAR' })}
                    />
                )}
