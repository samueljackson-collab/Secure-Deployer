...
export const CredentialsForm: React.FC<CredentialsFormProps> = ({ credentials, setCredentials }) => {
    ...
    return (
        <form className="space-y-4">
            ...
            <div className="relative">
                <label htmlFor="biosPassword" className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-1 ml-1">
                    BIOS Password
                    <span className="ml-2 text-gray-600 normal-case font-normal">Optional — enables DCU unattended execution</span>
                </label>
                <div className="relative">
                    <input
                        ...
                        id="biosPassword"
                        autoComplete="off"
                        value={credentials.biosPassword ?? ''}
                        onChange={handleChange}
                        placeholder="Leave blank if no BIOS password is set"
                        className="w-full bg-black/40 border border-gray-800 rounded-xl px-4 py-3 text-sm text-gray-200 placeholder-gray-600 focus:ring-2 focus:ring-yellow-500/50 focus:border-yellow-500 transition-all outline-none"
                    />
                    <button
                        type="button"
                        onClick={() => setShowBiosPassword(!showBiosPassword)}
                        className="absolute inset-y-0 right-0 px-4 flex items-center text-gray-500 hover:text-yellow-400 transition-colors"
                    >
                        {showBiosPassword ? (
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-8.707l-3-3a1 1 0 00-1.414 0l-3 3a1 1 0 001.414 1.414L9 9.414V13a1 1 0 102 0V9.414l1.293 1.293a1 1 0 001.414-1.414z" clipRule="evenodd" />
