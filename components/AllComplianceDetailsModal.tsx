        ...
        <div className="border border-gray-800 rounded-lg">
            ...
            >
                ...
                <div className="flex items-center gap-4">
                    <span className={`font-bold ${overallStatusColor}`}>{result.status}</span>
...
interface AllComplianceDetailsModalProps {
    isOpen: boolean;
    onClose: () => void;
    devices: ImagingDevice[];
}

export const AllComplianceDetailsModal: React.FC<AllComplianceDetailsModalProps> = ({ isOpen, onClose, devices }) => {
    if (!isOpen) {
        return null;
    }

    const completedDevices = devices.filter(d => d.status === 'Completed' && d.complianceCheck);
            ...
            <div className="bg-gray-950 rounded-lg shadow-2xl border border-gray-700 w-full max-w-5xl m-4 flex flex-col max-h-[90vh]">
                <div className="p-4 border-b border-gray-800 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-xl font-bold text-gray-100">All Device Compliance Reports</h2>
                    </div>
                </div>
                <div className="p-6 flex-grow overflow-y-auto space-y-3">
                    {completedDevices.length > 0 ? (
                        ...
                        <p className="text-center text-gray-400">No completed devices with compliance checks available.</p>
                    )}
                </div>
